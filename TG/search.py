...
async def search(client, message):
  ...
  await retry_on_flood(message.reply_text)(
    "<blockquote><b>✲『SCANIME』✲ ENGINE</b>\n<i>Select search Engine...</i></blockquote>",
...
async def search_group(client, message):
  ...
  try:
    ...
    return await message.reply("<code>Format:- /search AnimeName </code>")
  ...
  async def search_and_collect(web, web_name):
      ...
      async with semaphore:
          try:
              ...
              try:
                  await sts.edit_text(
                      f"<blockquote><b>✲『SCANIME』✲ ENGINE</b></blockquote>\n"
...
async def plugin_handler(_, query):
  ...
  if webs == "all" and search_query is not None:
    results_data, found_txt, no_found_txt = await search_all(search_query, query.message)
  elif webs is not None and search_query is not None:
    await retry_on_flood(query.edit_message_text)(
      f"<blockquote><b>✲『SCANIME』✲ ENGINE</b>\n<i>Searching <b>{search_query}</b> on {webs.name}......</i></blockquote>"
    )
  ...
  if not results_data and webs:
    return await retry_on_flood(query.edit_message_text)(
      text=f"<blockquote><b>✲『SCANIME』✲ ENGINE</b>\n<i>No results found on {webs.name}. </i>\n\n<i>Try at Other Webs ...</i></blockquote>",
  ...
  if webs != "all":
    caption_text = f"<blockquote expandable><b>✲『SCANIME』✲ ENGINE</b>\n<i>Search <b>{search_query}</b> from <b>{webs.name}</b></i></blockquote>"
  else:
    caption_text = f"<blockquote><b>✲『SCANIME』✲ ENGINE</b>\n<i>Search <b>{search_query}</b> from <b>All</b></i></blockquote>"
...
async def back_webs_handler(_, query):
  await igrone_error(query.answer)()
  return await retry_on_flood(query.edit_message_text)(
    text="<blockquote><b>✲『SCANIME』✲ ENGINE</b>\n<i>Select search Engine...</i></blockquote>",
    reply_markup=plugins_list(),
  )
...
async def mul_handler(_, query):
  try:
    query_text = query.message.text.html if query.message.text else "<blockquote><b>✲『SCANIME』✲ ENGINE</b></blockquote>"
...
async def epk_back_handler(_, query):
  ...
  if rdata in back_cache:
    ...
    await retry_on_flood(query.edit_message_text)(
      text=f"<blockquote><b>✲『SCANIME』✲ ENGINE</b>\n<i>Search results from {webs.name}</i></blockquote>",
      reply_markup=InlineKeyboardMarkup(button)
    )
  else:
